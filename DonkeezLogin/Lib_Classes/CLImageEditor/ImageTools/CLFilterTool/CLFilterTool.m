...
+(NSArray *)GetFilterClassNameArr{
    ...
    return @[@"CLDefaultEmptyFilter", @"CLDefaultLinearFilter", @"CLDefaultVignetteFilter", @"CLDefaultInstantFilter", @"CLDefaultTransferFilter", @"CLDefaultProcessFilter", @"CLDefaultSepiaFilter", @"CLDefaultChromeFilter", @"CLDefaultFadeFilter", @"CLDefaultCurveFilter", @"CLDefaultTonalFilter", @"CLDefaultNoirFilter", @"CLDefaultMonoFilter", @"CLDefaultInvertFilter"];
...
+(void)GetFilteredImageArr:(UIImage*)originImage completion:(void(^)(NSArray *images))completion{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        NSArray * filterName = [CLFilterTool GetFilterClassNameArr];
        NSMutableArray * filteredImageArr = [NSMutableArray new];
        for (NSString * each in filterName) {
            @autoreleasepool {
                Class filterClass = NSClassFromString(each);
                if([(Class)filterClass conformsToProtocol:@protocol(CLFilterBaseProtocol)]){
                    [filteredImageArr addObject:[filterClass applyFilter:originImage]];
                }else{
                    [filteredImageArr addObject:originImage];
                }
            }
        }
        
        completion(filteredImageArr);
