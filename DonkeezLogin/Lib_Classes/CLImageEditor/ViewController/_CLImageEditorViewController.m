...
//#import "PPPrivatePostViewController.h"

#pragma mark- _CLImageEditorViewController

@interface _CLImageEditorViewController()
<CLImageToolProtocol, UINavigationBarDelegate, UIAlertViewDelegate>
{
    PPSendOptionViewController *optVC;
}
...
//@property(nonatomic,strong)PPAvatarImageView *avaImgView;
...
- (void)initNavigationBar
{
    ...
    [[UINavigationBar appearance] setTitleTextAttributes:@{NSForegroundColorAttributeName:[UIColor whiteColor]}];
      ...
      [[UINavigationBar appearance] setBackgroundImage:[GlobalDefine imageWithColor:[UIColor blackColor] withFrame:CGRectMake(0, 0, 320, 65)] forBarMetrics:UIBarMetricsDefault];
    
    //-------- begin  ,  nav's right button set!
    UIButton *rightButton=[UIButton buttonWithType: UIButtonTypeCustom];
    [rightButton setFrame:CGRectMake(0, 0, 20, 20)];
    [rightButton addTarget:self action:@selector(pushedFinishBtn:) forControlEvents:UIControlEventTouchUpInside];
    [rightButton setImage:[UIImage imageNamed:@"icon_apply.png"] forState:UIControlStateNormal];
    
    UIBarButtonItem *rBtn=[[UIBarButtonItem alloc]initWithCustomView:rightButton];
    
    self.navigationItem.rightBarButtonItem = rBtn;
    
    //------- end   ------------
    
    //-------- begin  ,  nav's left button set!
    UIButton *leftButton=[UIButton buttonWithType: UIButtonTypeCustom];
    [leftButton setFrame:CGRectMake(0, 0, 16, 16)];
    [leftButton addTarget:self action:@selector(pushedCloseBtn:) forControlEvents:UIControlEventTouchUpInside];
    [leftButton setImage:[UIImage imageNamed:@"icon_cancel.png"] forState:UIControlStateNormal];
    
    UIBarButtonItem *lBtn=[[UIBarButtonItem alloc]initWithCustomView:leftButton];
    ...
    if(_navigationBar==nil){
        ...
        dy+=[UIApplication sharedApplication].statusBarFrame.size.height;
        ...
        dy=10;
        UINavigationBar *navigationBar = [[UINavigationBar alloc] initWithFrame:CGRectMake(0, dy, self.view.width, navHeight)];
...
- (void)initMenuScrollView
{
    if(self.menuView==nil){
        UIScrollView *menuScroll = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, self.view.width, menuheight)];
...
- (void)initImageScrollView
{
    if(_scrollView==nil){
        ...
        dy=10;
        imageScroll.top = dy+navHeight;
        
        
        imageScroll.height = self.view.height-dy - menuheight-navHeight ;
        
        imageScroll.backgroundColor=[CLImageEditorTheme toolbarColor];
...
- (void)viewDidLoad
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(onNotificationOptionSelect:) name:NotificationOptionPublicOrPrivate object:nil];
...
- (void)swapToolBarWithEditting:(BOOL)editting
{
    ...
    if(self.currentTool){
        ...
        UIButton *rightButton=[UIButton buttonWithType: UIButtonTypeCustom];
        [rightButton setFrame:CGRectMake(0, 0, 20, 20)];
        [rightButton addTarget:self action:@selector(pushedDoneBtn:) forControlEvents:UIControlEventTouchUpInside];
        [rightButton setImage:[UIImage imageNamed:@"icon_apply.png"] forState:UIControlStateNormal];
        ...
        UIButton *leftButton=[UIButton buttonWithType: UIButtonTypeCustom];
        [leftButton setFrame:CGRectMake(0, 0, 16, 16)];
        [leftButton addTarget:self action:@selector(pushedCancelBtn:) forControlEvents:UIControlEventTouchUpInside];
        [leftButton setImage:[UIImage imageNamed:@"icon_cancel.png"] forState:UIControlStateNormal];
...
-(void)onNotificationOptionSelect:(NSNotification*)notify
{
    NSString *opt=[notify.userInfo objectForKey:@"opt"];
   
    
    if([opt isEqualToString:@"public"]){
        
        dispatch_async(dispatch_get_main_queue(), ^{
           
            
            [self.delegate imageEditor:self didFinishEdittingWithImage:_originalImage isPrivate:NO];
            
            
        });
        
        
    }else if([opt isEqualToString:@"private"])
    {
         [self.delegate imageEditor:self didFinishEdittingWithImage:_originalImage isPrivate:YES];
    }
...
- (void)pushedFinishBtn:(id)sender
{
    ...
    if(self.targetImageView==nil){
        ...
        if([self.delegate respondsToSelector:@selector(imageEditor:didFinishEdittingWithImage:isPrivate:)]){
            
            NSString *outputPath = PATH_TO_PHOTO;
            
            UIImage *image = _imageView.image;
            CGFloat qsize = MIN(image.size.height, image.size.width);
            CGRect rect = CGRectMake(ceil((image.size.width-qsize)*.5), ceil((image.size.height-qsize)*.5), qsize,qsize);
            UIImage *croped = [image cropedToRect:rect];
            NSData* outdata = UIImageJPEGRepresentation(croped, .6);
            [outdata writeToFile:outputPath atomically:NO];
            
         
//            UIColor *hiCol= [UIColor colorWithRed:104.0/255.0 green:205.0/255.0 blue:49.0/255.0 alpha:1.0];
            
            UIAlertView* av = [[UIAlertView alloc] initWithTitle:@"Post to Relationship" message:@"Are you sure?" delegate:self cancelButtonTitle:@"CANCEL" otherButtonTitles:@"OK",nil];
            av.tag = 102;
            
            
//            if(appdelegate.appState==ASBARRELATION)
//            {
//                [av show];
//            }else if(appdelegate.appState==ASBARJOURNAL){
...
//                [self showMenu:(UIView*)self.imageView];
...
//-(void)onMakeProfile{
//    
//    NSData* rdata = UIImageJPEGRepresentation(_originalImage, DEFAULT_JPEG_QUALITY);
//    PFFile* file = [PFFile fileWithName:@"photo.jpg" data:rdata];
//    PFUser * curUser = [PFUser currentUser];
//    curUser[kPPImage]=file;
//    
//    UIActivityIndicatorView *av=activityViewToView(self.view);
//
...
//-(void)onPrivatePost
//{
//    PPPrivatePostViewController *ppVC=[[PPPrivatePostViewController alloc]init];
//    [ppVC setModalPresentationStyle:UIModalPresentationOverCurrentContext];
//    [ppVC setModalTransitionStyle:UIModalTransitionStyleCrossDissolve];
//    
...
//{
//    
//    KxMenuItem *makeProfile  =  [KxMenuItem menuItem:@"Make Profile Picture"
//                                                 image:[UIImage imageNamed:@"profile_line"]
//                                                target:self
//                                                action:@selector(onMakeProfile)];
//    KxMenuItem *privataeMenuItem  =  [KxMenuItem menuItem:@"Send as Private Picture"
//                                                  image:[UIImage imageNamed:@"img_private_white"]
//                                                 target:self
//                                                 action:@selector(onPrivatePost)];
//    KxMenuItem *publicMenuItem  =  [KxMenuItem menuItem:@"Send as Public Picture"
//                                                  image:[UIImage imageNamed:@"img_public_white"]
//                                                 target:self
//                                                 action:@selector(onPublicPost)];
//
//    NSMutableArray *menuItems = [[NSMutableArray alloc] init];
//
//
//   [menuItems addObject:makeProfile];
//    [menuItems addObject:privataeMenuItem];
//    [menuItems addObject:publicMenuItem];
//    
//
//    
