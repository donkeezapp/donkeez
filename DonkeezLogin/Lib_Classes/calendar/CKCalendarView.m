...
- (void)_init:(CKCalendarStartDay)firstDay {
    ...
    yearLabel.autoresizingMask = UIViewAutoresizingFlexibleBottomMargin | UIViewAutoresizingFlexibleWidth;
    ...
    //    UIView *calendarContainer = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 280, 280)];
    ...
    for (int i = 0; i < 7; ++i) {
        ...
        [dayOfWeekLabel setTextColor:[UIColor redColor]];
...
- (void)layoutSubviews {
    ...
    self.yearLabel.text = [NSString stringWithFormat:@"%ld" ,(long)[GlobalDefine getYearFromDate:_monthShowing]];
    self.yearLabel.textColor = BD_HILIGHTED_MAIN_COLOR;
    [self.yearLabel setFont:[UIFont systemFontOfSize:20]];
    self.yearLabel.frame = CGRectMake(0, 0, self.bounds.size.width, YEAR_LABEL_HEIGHT);
    
    NSInteger titleLabelWidth = self.bounds.size.width - 48 * 2 - BUTTON_MARGIN * 2 ;
    NSInteger titleX = (self.bounds.size.width - titleLabelWidth) / 2.;
    self.titleLabel.text = [GlobalDefine getMonthNameWithDate:_monthShowing];
    self.titleLabel.frame = CGRectMake(titleX, YEAR_LABEL_HEIGHT + 1, titleLabelWidth, TOP_HEIGHT);
    [self.titleLabel setFont:[UIFont systemFontOfSize:20]];
    [GlobalDefine makeCornerRadius:self.titleLabel Radius: TOP_HEIGHT / 2. borderColor:BD_POPVIEW_BACK_COLOR backgroundColor:BD_POPVIEW_BACK_COLOR];
    
    
    self.prevButton.frame = CGRectMake(BUTTON_MARGIN, BUTTON_MARGIN + YEAR_LABEL_HEIGHT, 48, 38);
    self.nextButton.frame = CGRectMake(self.bounds.size.width - 48 - BUTTON_MARGIN, BUTTON_MARGIN + YEAR_LABEL_HEIGHT, 48, 38);
    ...
    while ([date laterDate:endDate] != date) {
        ...
        NSLog(@"btn_date : %@",   dateButton.date);
        dateButton.userInteractionEnabled = YES;
        [dateButton addTarget:self action:@selector(_dateButtonPressed:) forControlEvents:UIControlEventTouchUpInside];
        CKDateItem *item = [[CKDateItem alloc] init];
        item.textColor = UIColorFromRGB(0x5d305b);
        item.backgroundColor = BD_HILIGHTED_MAIN_COLOR;
        
        
        
        if ([self _dateIsToday:dateButton.date]) {
            //            item.textColor = UIColorFromRGB(0x28ee28);
            //            item.backgroundColor = BD_POPVIEW_BACK_COLOR;
            //
        } else if (!self.onlyShowCurrentMonth && [self _compareByMonth:date toDate:self.monthShowing] != NSOrderedSame) {
            //            item.textColor = [UIColor lightGrayColor];
            //            item.backgroundColor = BD_HILIGHTED_MAIN_COLOR;
            item.backgroundColor = BD_HILIGHTED_MAIN_COLOR;
            item.textColor = myRGBA(113, 69, 109, 255) ;
            
        }
        if([self _placeInWeekForDate:dateButton.date] == 6){
            item.backgroundColor = BD_POPVIEW_BACK_COLOR;
            item.textColor = BD_HILIGHTED_MAIN_COLOR ;
        }
        ...
        if(![[GlobalDefine getMonthNameWithDate:_monthShowing] isEqualToString:[GlobalDefine getMonthNameWithDate:dateButton.date]]){
            NSLog(@"date userInteractionEnabled >>> %@", dateButton.date);
            dateButton.userInteractionEnabled = NO;
            dateButton.backgroundColor = BD_MAIN_BACK_COLOR;
            [dateButton setTitleColor:BD_POPVIEW_BACK_COLOR forState:UIControlStateNormal];
